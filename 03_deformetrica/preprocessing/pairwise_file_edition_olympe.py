...
def write_launch_simulation_pairwise_chdb_sh(root_directory, xml_parameters):
    ...
    file.write("--command-line  \"(cd %%in-dir%%/%%dirname%% ; export CUDA_VISIBLE_DEVICES=\$(expr \$CHDB_RANK %% %s); "
               "$DEFORMETRICA estimate model.xmli data_set.xml -p $OPTIMIZATIONPARAMETERS --output=output > logout.txt)\"\n" % xml_parameters.number_of_gpus)
    file.close()


def pairwise_file_edition(root_directory=None, output_folder_name=None, subject_ids=None, visit_ages=None, object_id=None,
                       model_type=None, dimension=None, deformable_object_type=None, attachment_type=None,
                       noise_std=None, object_kernel_type=None, object_kernel_width=None,deformation_kernel_width=None,
    ...
    if number_of_memory is None:
        ...
        number_of_memory = simpledialog.askstring("Input", "What is the memory?",
                                                        initialvalue="96000", parent=root)
    ...
    for vtk_file1, indice1 in zip(list_vtk, range(len(list_vtk))):
        for vtk_file2, indice2 in zip(list_vtk, range(len(list_vtk))):
            if vtk_file1!=vtk_file2:
                xml_parameters.sourceFile = vtk_file1.name
                xml_parameters.targetFile = vtk_file2.name
                folder_name = "%s_to_%s" % (vtk_file1.name[0:(len(vtk_file1.name)-4)], vtk_file2.name[0:(len(vtk_file2.name)-4)])
    ...
    write_launch_simulation_pairwise_chdb_sh(output_folder_name, xml_parameters)
