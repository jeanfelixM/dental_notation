...
def write_vtk(imageData,basename,dirname):
    
    Path(path.join(dirname, "calcul")).mkdir(parents=True, exist_ok=True)
    Path(path.join(dirname, "calcul/surfaces")).mkdir(parents=True, exist_ok=True)
    
    # Écriture des données dans un fichier VTK
    writer = vtk.vtkPolyDataWriter()
    writer.SetFileVersion(42)
    writer.SetInputData(imageData)
    writer.SetFileName(path.join(dirname, "calcul/surfaces",path.splitext(basename)[0]) + ".vtk")
    writer.Write()
    
def filter_and_rewrite(filename,basename,dirname):
    ...
    write_vtk(imageData,basename,dirname)
